      ...
      <div className="max-w-lg mx-auto px-4 pt-20 text-center">
        ...
        >
          ...
          <p className="text-sm text-brand-muted">
            Connect your Solana wallet to link external wallets and boost your credit score.
          </p>
        </motion.div>
      </div>
    );
    ...
    <div className="max-w-2xl mx-auto px-4 pt-8 pb-20">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-brand-muted">
              Powered by Ika dWallet MPC Infrastructure
            </p>
          </div>
        </div>
      </motion.div>
      ...
      {totalBoost > 0 && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="mt-6 p-4 rounded-2xl border border-teal-500/30 bg-teal-500/5"
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-white">
                  Cross-Chain Boost Active
                </p>
                <p className="text-xs text-brand-muted">
                  Score includes activity from {ika.connectedChains.length} external wallet{ika.connectedChains.length > 1 ? 's' : ''}
                </p>
              </div>
            </div>
        ...
        >
          ...
          {ika.connectedChains.map((chain, i) => (
            <motion.div
              ...
              className="bg-brand-card rounded-xl border border-brand-border p-4"
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div
                    className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-white"
                    style={{ background: chain.chain === 'ETH' ? '#627EEA20' : '#F7931A20' }}
                  >
                    <span style={{ color: chain.chain === 'ETH' ? '#627EEA' : '#F7931A' }}>
                      {chain.chain}
                    </span>
                  </div>
                  <div>
                ...
                <button
                  onClick={() => ika.disconnectChain(chain.address, chain.chain)}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-brand-bg/50 rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[10px] text-brand-muted">Wallet Age</span>
                  </div>
                ...
                <div className="bg-brand-bg/50 rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[10px] text-brand-muted">Tx Volume</span>
                  </div>
                ...
                <div className="bg-brand-bg/50 rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[10px] text-brand-muted">Total Boost</span>
                  </div>
      ...
      <AnimatePresence>
        {result && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0 }}
            className="mt-6 bg-brand-card rounded-2xl border border-green-500/30 p-5"
          >
            ...
            <p className="text-xs text-brand-muted mb-3">
              Your {result.chain} wallet has been linked via Ika dWallet MPC. Score boost: +{result.totalBoost} pts.
            </p>
            {result.txSignature && (
              <a
              ...
              >
                View binding transaction on Solscan
                <ExternalLink className="w-3 h-3" />
              </a>
            )}
            <button
              onClick={() => setResult(null)}
              className="block mt-2 text-xs text-brand-muted hover:text-white transition-colors"
            >
              Dismiss
            </button>
          </motion.div>
      ...
      {/* Add External Wallet Form */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.2 }}
        className="mt-6 bg-brand-card rounded-2xl border border-brand-border p-6"
      >
        <h3 className="text-base font-bold text-white mb-1">Add External Wallet</h3>
        <p className="text-xs text-brand-muted mb-5">
          Connect an ETH or BTC wallet via Ika's MPC signing infrastructure. Lendra reads its
          transaction history and includes it in your credit score.
        </p>
        ...
        {/* Address Input */}
        <div className="mb-4">
          <label className="block text-xs text-brand-muted mb-2">
        ...
        <button
          ...
          disabled={!externalAddress.trim() || ika?.isBinding}
        ...
        {ika?.bindingTx && (
          <div className="mt-3 text-center">
            <a
              href={`https://solscan.io/tx/${ika.bindingTx}?cluster=devnet`}
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs text-brand-muted hover:text-brand-accent transition-colors inline-flex items-center gap-1"
            >
              Last binding tx: {ika.bindingTx.slice(0, 12)}...
              <ExternalLink className="w-3 h-3" />
            </a>
          </div>
        )}
          ...
          {[
            {
              ...
              desc: 'Ika creates a dWallet key pair via 2PC-MPC distributed key generation. No private keys leave your device.',
            },
            {
              icon: Link2,
              title: 'Identity Binding',
              desc: 'Your external wallet is cryptographically bound to your Solana address. Verified on-chain, no bridges needed.',
            },
            {
              icon: Activity,
              title: 'Activity Analysis',
              desc: 'Lendra reads the external wallet history: age, volume, and behavior. Up to +50 bonus points.',
            },
            {
              icon: Zap,
              title: 'Score Boost',
              desc: 'External wallet age adds up to +30 pts. Transaction volume adds up to +20 pts. Max cross-chain boost: +50 pts.',
            },
          ].map((step, i) => (
